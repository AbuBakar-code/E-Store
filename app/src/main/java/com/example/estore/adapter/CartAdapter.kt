...
import com.example.estore.databinding.ViewholderCartBinding
...
class CartAdapter(private val listItemSelected: ArrayList<ItemsModel>, context: Context, var changeNumberItemsListener: ChangeNumberItemsListener? = null): RecyclerView.Adapter<CartAdapter.CartViewHolder>() {
    ...
    override fun onBindViewHolder(holder: CartAdapter.CartViewHolder, position: Int) {
        ...
        Glide.with(holder.itemView.context).load(item.picUrl).apply(RequestOptions().transform(CenterCrop())).into(holder.binding.pic)
